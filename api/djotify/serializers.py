...
class PlaylistSongSerializer(serializers.ModelSerializer):
    song = SongListingField(queryset=Song.objects.all(), required=False)
    playlist = PlaylistListingField(queryset=Playlist.objects.all(), required=False)

    class Meta:
        model = PlaylistSong
        fields = ["track_num", "playlist", "song"]


class PlaylistKeywordSerializer(serializers.ModelSerializer):
    playlist = PlaylistListingField(queryset=Playlist.objects.all(), required=False)
    keyword = KeywordListingField(queryset=Playlist.objects.all(), required=False)
